...
import { VisualizerConfig } from "./index";
...
const SegmentTextureControls = ({
  ...
  <div className="mt-2 rounded-lg border border-border/50 p-3 space-y-2 bg-secondary/30">
    {config.allowScaling && (
      <TextureSlider
        label="Scale"
        value={tex.scale}
...
export default function VisualizerMain({
  ...
  const handleMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const idx = getHoverIndex(e, canvasRef.current, imageSize.width, imageSize.height);
  ...
  return (
    <div className="fixed z-[900] inset-0 bg-background">
      <div className="flex flex-col-reverse lg:flex-row h-screen w-screen gap-0">
        {/* Sidebar */}
        <aside className="w-full lg:w-80 h-full border-r border-border/50 bg-card/50 backdrop-blur-sm p-4 pb-20 space-y-3 overflow-y-auto">
          <div className="flex justify-between gap-2">
            <img
              src={config.companyLogo}
              className="h-9 max-w-36 object-contain rounded-lg"
            />
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-40 p-1 z-[910]">
                  <button
                    ...
                    onClick={() =>
                      canvasRef.current && downloadRenderedImage(canvasRef.current)
                    }
                  >
                    Download image
                  </button>
          ...
          {segments.map((seg, i) => (
            <div key={i} className="border border-border/50 rounded-lg overflow-hidden bg-secondary/20 hover:border-primary/30 transition-colors">
              <div
                className="p-3 cursor-pointer font-medium flex justify-between items-center"
              ...
              >
                ...
                {seg.texture && (
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      removeSegmentTexture(seg.segment_id);
                    }}
                  >
                    <Trash2 />
                  </Button>
        ...
        <main className="flex-1 max-h-[50dvh] lg:max-h-full h-full">
          <div className="relative h-full overflow-hidden">
            <canvas
              ...
              style={{
                pointerEvents: isBundleLoading || isTextureLoading ? "none" : "auto",
              }}
            />
          ...
          <TextureSelector
            ...
            onSelect={(file, scale) =>
              selectedSeg != null && applyTextureToSegment(selectedSeg, file, scale)
            }
          />
        </main>
      </div>
